...
const async_waterfall = require('async/waterfall');
const async_map = require('async/map');
const async_parallel = require('async/parallel');
const async_apply = require('async/apply');
const async_mapLimit = require('async/mapLimit');
...
internals.callBeforeHooks = (table, name, startFun, callback) => {
  ...
  return async_waterfall([startFun].concat(listeners), callback);
};
...
Table.prototype.create = function (item, options, callback) {
  ...
  if (_.isArray(item)) {
    async_map(item, (data, callback) => internals.createItem(self, data, options, callback), callback);
  } else {
...
Table.prototype.updateTable = function (throughput, callback) {
  ...
  async_parallel([
    async_apply(internals.syncIndexes, self),
    async_apply(internals.updateTableCapacity, self, throughput),
  ], callback);
};
...
internals.syncIndexes = (table, callback) => {
  ...
  table.describeTable((err, data) => {
    ...
    async_mapLimit(missing, maxIndexCreationsAtaTime, (params, callback) => {
      const attributeDefinitions = [];
