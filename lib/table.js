...
Table.prototype.sendRequest = function (method, params, callback) {
  ...
  self.log && self.log.info({ params: params }, 'dynogels %s request', method.toUpperCase());
  driver[method].call(driver, params, (err, data) => {
    const elapsed = Date.now() - startTime;

    if (err) {
      self.log && self.log.warn({ err: err }, 'dynogels %s error', method.toUpperCase());
      return callback(err);
    } else {
      self.log && self.log.info({ data: data }, 'dynogels %s response - %sms', method.toUpperCase(), elapsed);
