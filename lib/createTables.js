...
internals.createTable = (model, globalOptions, options, callback) => {
  ...
  model.describeTable((err, data) => {
    if (_.isNull(data) || _.isUndefined(data)) {
      model.log && model.log.info('creating table: %s', tableName);
      return model.createTable(options, error => {
        if (error) {
          model.log && model.log.warn({ err: error }, 'failed to create table %s: %s', tableName, error);
          return callback(error);
        }

        model.log && model.log.info('waiting for table: %s to become ACTIVE', tableName);
        internals.waitTillActive(globalOptions, model, callback);
      });
    } else {
      model.updateTable(err => {
        if (err) {
          model.log && model.log.warn({ err: err }, 'failed to update table %s: %s', tableName, err);
          return callback(err);
        }
